...
func init() {
	rh := RuleHandler{
		Rule: driverV2.Rule{
			Name:       SQLE00014,
			Desc:       "尽量避免使用自定义函数",
			Annotation: "自定义函数在一定程度上能使程序难以调试和拓展，各种数据库端的存储过程语法相差很大，给将来的数据移植带来很大的困难，且会极大的出现BUG的几率",
			Level:      driverV2.RuleLevelError,
			Category:   RuleTypeDDLConvention,
			Params:     nil,
		},
		Message:              "尽量避免使用自定义函数",
		RawSQLHandler:        RuleSQLE00014,
		AllowOffline:         true,
...
/*
==== Prompt start ====
在 TBase(PostgreSQL的分布式版本) 中，您应该检查 SQL 是否违反了规则(SQLE00014): "在 TBase 中，尽量避免使用自定义函数."
您应遵循以下逻辑：
...
// ==== Rule code start ====
// 规则函数实现开始
func RuleSQLE00014(ctx context.Context, rule *driverV2.Rule, sql string, nextSQL []string) (string, error) {
	// 解析 SQL 语句
